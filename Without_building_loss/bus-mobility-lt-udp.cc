...
void MyAPPC::ScheduleTx(void) 
{
  if (m_running) 
      ...
      */
      m_sendEvent = Simulator::Schedule(Seconds(0.01), &MyAPPC::SendPacket, this);
	...
	if (type == Data_Pks_Header && !m_OK ) // check if it is data packet and if we have decoded the original file
	  {
	     ...
	     if (m_receivePos > m_decodethreshold && m_packetsReceive % 100 == 0 )
		{
		  ...
		  if (LT_Decode(buf,m_receiveBuffer,filelen,m_receivePos))
		     {
		       std::ofstream freceived("new_128.txt",std::ofstream::binary);
...
/*-----------------------------------------------------------------------------------------------------------*/
  for(int i =1; i<30; i++)
  {
  Address sinkAddress(InetSocketAddress (VANET::m_interfaces.GetAddress(i), m_sinkPort)); //i
  Ptr<MyAPPS> appS = CreateObject<MyAPPS>();
  appS->Setup(sinkAddress, m_sinkPort, Pks_Len, DataRate("5Mbps"));
  VANET::m_vanetTxNodes.Get(i)->AddApplication(appS);    //i
  appS->SetStartTime(Seconds(0.));
  appS->SetStopTime(Seconds(40.));

  // the client part
  Ptr<Socket> ns3UdpSocket = Socket::CreateSocket(VANET::m_vanetTxNodes.Get(0),UdpSocketFactory::GetTypeId());
  ...
  VANET::m_vanetTxNodes.Get(0)->AddApplication(appC);
  appC->SetStartTime(Seconds(1.));
  appC->SetStopTime(Seconds(40.));
}
...
int main(int argc, char *argv[])
{
  ...
  std::cout << "Usage of  " << argv[0] << " :\n\n"
            "./waf --run '/scratch/1-6-bus-mobility --PrintHelp' --vis \n" 
